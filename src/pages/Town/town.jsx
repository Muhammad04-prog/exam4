...
import heroBg from "../../assets/images/Rectangle 37 (2).png";
  ...
  useEffect(() => {
    const getHouses = async () => {
      try {
        ...
        setHouses((data.townhouses || []).map(h => ({ ...h, id: `town-${h.id}` })));
      } catch (err) {
      ...
      <Container maxWidth="lg" sx={{ mt: 6, mb: 10 }}>
        ...
        >
          <Stack direction="row" spacing={2} mb={4} sx={{ flexWrap: 'wrap', gap: 1 }}>
            ...
            <FilterTab label="Таунхаусы" active />
          ...
          <Breadcrumbs sx={{ mb: 4, fontSize: '14px', color: '#999' }}>
            ...
            <Typography color="text.primary" sx={{ fontSize: '14px' }}>Таунхаусы</Typography>
          </Breadcrumbs>
          <Divider sx={{ mb: 6 }} />
        </motion.div>

        <AnimatePresence mode='wait'>
          <Grid container spacing={4} sx={{ mb: 6 }} key="townhouses-grid">
            {houses.map((house) => (
