...
import heroBg from "../../assets/images/Rectangle 37 (7).png";
  ...
  useEffect(() => {
    const getHouses = async () => {
      try {
        ...
        setHouses((data.twoStoryHouses || []).map(h => ({ ...h, id: `two-${h.id}` })));
      } catch (err) {
          ...
          <Container maxWidth="lg">
             ...
             >
               <Typography variant="h3" sx={{ fontWeight: 800, textTransform: "uppercase", color: 'white' }}>
                 ДВУХЭТАЖНЫЕ <Box component="span" sx={{ color: '#F2A359' }}>ДОМА</Box>
               </Typography>
             </motion.div>
          </Container>
        </Box>
      ...
      <Container maxWidth="lg" sx={{ mt: 6, mb: 10 }}>
        ...
        >
          <Stack direction="row" spacing={2} mb={4} sx={{ flexWrap: 'wrap', gap: 1 }}>
            <FilterTab label="1 этажные" />
            <FilterTab label="2 этажные" active />
          ...
          <Breadcrumbs sx={{ mb: 4, fontSize: '14px', color: '#999' }}>
            ...
            <Typography color="text.primary" sx={{ fontSize: '14px' }}>Двухэтажные дома</Typography>
          </Breadcrumbs>
          <Divider sx={{ mb: 6 }} />
        </motion.div>

        <AnimatePresence mode='wait'>
          <Grid container spacing={4} sx={{ mb: 6 }} key="two-story-grid">
            {houses.map((house) => (
